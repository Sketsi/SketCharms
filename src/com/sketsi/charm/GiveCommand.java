...
public class GiveCommand implements CommandExecutor {
	...
	/*
	 *  /give player <Charm Name/Potion> <amount>
	 *  //Lore to check for
	...
	/* 		If Command is less than 2 arguements, but less than 3 - > See if player is online else check username
	 *  	Arguments: 1. Player 2. CharmName/Effect 3. Quantity
	 * 		
	 */
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if ((args.length >= 1) && (args.length <= 2)) {
			...
			if (Bukkit.getPlayerExact(args[0]) != null) {
				...
				if (Bukkit.getPlayerExact(args[1]) != Charms); {
					player.sendMessage("Hi");
					
				}
				
			} else {
					player.sendMessage(ChatColor.GOLD + Prefix + ": That charm doesn't exist!");
				}	player.sendMessage(ChatColor.GOLD + Prefix + ": Check the username in your command.");
				
			} else {
			player.sendMessage(ChatColor.GOLD + Prefix + ": /give <Player> <Charm Name/Potion> <Quantity>");
		}
		return false;
	}
